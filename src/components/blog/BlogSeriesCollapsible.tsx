...
interface BlogSeriesCollapsibleProps {
  title: string;
...
export function BlogSeriesCollapsible({ title, posts, numbered = true, className }: BlogSeriesCollapsibleProps) {
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className={`blog-perps-collapsible ${className ?? ""}`}>
      <CollapsibleTrigger className="blog-perps-trigger">
        ...
        <span className="blog-perps-trigger-text">{title}</span>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <Link href={`/blog/${post.slug}`} className="blog-list-title">
                    {numbered && <span className="blog-perps-num">{i + 1}.</span>} {post.title}
                  </Link>
                  {post.excerpt && (
